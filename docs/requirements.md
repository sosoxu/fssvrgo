...
- 取消后的会话不得继续接收数据块；进度接口对已取消顺序上传返回 0，终态账本可独立保留取消前的已确认字节数用于审计
...
- **列表分页性能**：默认列表请求不执行全量 `COUNT`，通过 `HasMore` 判断后续页；需要精确总数时必须显式传 `include_total=true`
...
发布门禁使用固定环境连续运行至少 3 轮，错误率必须为 0；时间目标以 3 轮 p95 判定。仓库已增加 `TestFileIOPerformance` 服务层基线，覆盖 1K、1M、100M、1G 的串行/并发读写并输出原始样本和分位数；共享读 lease、事务内释放和临时网络错误重试已使 2026-08-27 本机完整负载序列达到 48/48 成功。该基线不经过 HTTP/gRPC，且 1G 冷读 p95 仍受磁盘缓存和前序写入影响，不能替代上述协议目标门禁，因此本项状态为“部分完成、待验收”。
...
| 1GB 性能发布门禁 | 已有可配置的 3 轮服务层基线和报告，修复后完整负载错误率为 0；HTTP/gRPC 固定环境及冷缓存 p95 门禁仍缺失 | R-11 |
