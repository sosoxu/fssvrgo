...
**审计日志写入与可见性**：`audit_log` 由 `AuditWriter` 异步批量写入（缓冲满批或按间隔刷盘，
进程崩溃会丢失未刷盘条目，这是刻意的取舍）。查询接口 `GET /api/v1/audit-logs` 在读库前会先
调用 `AuditWriter.Flush`，因此"刚执行完操作立刻查询"能看到该记录，不会与刷盘周期竞争。
